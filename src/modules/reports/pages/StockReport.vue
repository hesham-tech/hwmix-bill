<template>
  ...
  >
      ...
      <v-row dense>
        <v-col cols="12" sm="4">
          <v-card border flat class="rounded-md pa-4 bg-primary-lighten-5 border-primary">
            ...
            <div class="text-h4 font-weight-bold text-primary">{{ formatCurrency(summary.total_cost_value) }}</div>
          </v-card>
        </v-col>
        <v-col cols="12" sm="4">
          <v-card border flat class="rounded-md pa-4 bg-success-lighten-5 border-success">
            ...
            <div class="text-h4 font-weight-bold text-success">{{ formatCurrency(summary.total_sale_value) }}</div>
          </v-card>
        </v-col>
        <v-col cols="12" sm="4">
          <v-card border flat class="rounded-md pa-4 bg-info-lighten-5 border-info">
            ...
            <div class="text-h4 font-weight-bold text-info">{{ formatCurrency(summary.potential_profit) }}</div>
          </v-card>
        </v-col>
      </v-row>
    </template>
    ...
    <template #charts>
      <v-row>
        ...
        <v-col cols="12" md="6">
          <v-card border flat class="rounded-md pa-4 fill-height">
            <h3 class="text-h6 font-weight-bold mb-4">تنبيهات حالة المخزون</h3>
  ...
  <div v-else class="pa-4 text-center d-flex flex-column align-center justify-center" style="min-height: 400px">
    ...
    <p class="text-body-1 text-grey mb-2">ليس لديك إذن للوصول إلى تقارير تقييم المخزون. يرجى مراجعة المسؤول.</p>
